  ...
  this.home  = function(req, res) {
      ...
      async.parallel([
          ...
          function topcountries(callback){
              ...
              sql = "SELECT CONCAT('"+config.RESOURCE_URL+"', tc.product_image) as product_image, tc.country_id, cl.country_name, cl.redirect_url, CONCAT('"+config.RESOURCE_URL+"',cl.country_flag) as country_flag, CONCAT('"+config.RESOURCE_URL+"',cl.company_logo) as company_logo, cl.country_domain, cl.preferred_currency_id FROM top_country tc JOIN country_list cl ON(tc.country_id = cl.id) WHERE tc.status = 1 ORDER BY tc.order_by ASC LIMIT 5";
          ...
          function orderByPhone(callback){
              ...
              sql = "SELECT country_name, CONCAT('"+config.RESOURCE_URL+"',country_flag) as country_flag,phone FROM country_list WHERE status = 1 AND is_display = 1 LIMIT 4";
          ...
          function countriesprovinces(callback) {
            var country = [];
              ...
              dbModel.rawQuery("SELECT id, country_name, redirect_url, TRIM(TRAILING ',' FROM CONCAT(country_name,',',country_alias)) as alias,short_code,iso_code,CONCAT('"+config.RESOURCE_URL+"',country_flag) as country_flag,CONCAT('"+config.RESOURCE_URL+"',company_logo) as company_logo, show_state, preferred_currency_id FROM country_list WHERE status = 1", function(err, countries) {
                if (err) return callback(err);
                else 
